...
impl ParsedDataItems {
    ...
    pub fn new_from_typed_data(
        ...
        let parse_result = data_iter
            .next()
            .ok_or(SequencerBlockError::no_rollup_transactions_root())?;
        let (rollup_txs_root_index, data_item) = parse_result?;
        let rollup_transactions_root = data_item
            .into_rollup_transactions_root(rollup_txs_root_index)
        ...
        let parse_result = data_iter
            .next()
            .ok_or(SequencerBlockError::no_rollup_ids_root())?;
        let (rollup_ids_root_index, data_item) = parse_result?;
        let rollup_ids_root = data_item
            .into_rollup_ids_root(rollup_ids_root_index)
            .map_err(SequencerBlockError::data_item)?;
        ...
        let maybe_upgrade_item = matches!(
            data_iter.peek(),
            Some(Ok((_index, DataItem::UpgradeChangeHashes(_))))
        )
        .then(|| {
        ...
        let maybe_extended_commit_info_item = with_extended_commit_info
            .then(|| {
                ...
                let encoded_extended_commit_info = data_item
        ...
        let rollup_transactions = data_iter
            .map(|result| {
                let bytes = result.and_then(|(index, data_item)| {
                    data_item
                        .into_rollup_transaction(index)
                        .map_err(SequencerBlockError::data_item)
                })?;
                merkle_tree_leaves.push(Sha256::digest(&bytes));
                let raw_tx = RawTransaction::decode(bytes)
                    .map_err(SequencerBlockError::transaction_protobuf_decode)?;
                Transaction::try_from_raw(raw_tx)
                    .map(Arc::new)
                    .map_err(SequencerBlockError::raw_signed_transaction_conversion)
