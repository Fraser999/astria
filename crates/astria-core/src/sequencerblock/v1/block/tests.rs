...
fn tx_legacy_bytes(nonce: u32) -> Bytes {
    tx(nonce).into_raw().encode_to_vec().into()
}

fn tx_bytes(nonce: u32) -> Bytes {
    DataItem::RollupTransaction(tx(nonce).into_raw().encode_to_vec().into())
        .encode()
        .unwrap()
}
...
mod parsed_data_items {
    ...
    fn should_fail_to_parse_data_items_rollup_ids_root_not_second_item() {
        let data = [rollup_txs_root_bytes(), tx_bytes(0)];
        let SequencerBlockError(error_kind) =
            ParsedDataItems::new_from_typed_data(&data, true).unwrap_err();
        assert_err_matches!(
            error_kind,
    ...
    #[test]
    fn should_fail_to_parse_data_items_containing_unexpected_extended_commit_info() {
        let data = [
        ...
        let SequencerBlockError(error_kind) =
            ParsedDataItems::new_from_typed_data(&data, false).unwrap_err();
        assert_err_matches!(
            error_kind,
            SequencerBlockErrorKind::DataItem(DataItemError(DataItemErrorKind::Mismatch { index, .. }))
            if index == 2
    ...
    #[test]
    fn should_fail_to_parse_data_items_containing_stray_item_in_rollup_txs() {
        let data = [
            rollup_txs_root_bytes(),
            rollup_ids_root_bytes(),
            minimal_extended_commit_info_bytes(),
    ...
    fn should_fail_to_parse_data_items_malformed_protobuf_tx() {
        let data = [
            ...
            rollup_ids_root_bytes(),
            minimal_extended_commit_info_bytes(),
            DataItem::RollupTransaction(vec![3].into())
                .encode()
                .unwrap(),
        ];
        let SequencerBlockError(error_kind) =
    ...
    fn should_fail_to_parse_data_items_malformed_raw_tx() {
        ...
        let data = [
            ...
            rollup_ids_root_bytes(),
            minimal_extended_commit_info_bytes(),
            DataItem::RollupTransaction(bad_tx.encode_to_vec().into())
                .encode()
                .unwrap(),
        ];
        let SequencerBlockError(error_kind) =
