...
impl SubmissionState {
    ...
    ///
    /// Returns an error if `new_sequencer_height` is not greater than the last confirmed submitted
    /// sequencer height or if writing fails.
    pub(super) async fn start(
        mut self,
        new_sequencer_height: SequencerHeight,
    ) -> eyre::Result<Self> {
        if let Some(last_completed_submission) = self.last_completed_submission() {
            ensure!(
                new_sequencer_height > last_completed_submission.sequencer_height,
        ...
        self.state = State::new_started(new_sequencer_height, last_submission);
    ...
    ///
    /// Returns an error if state is anything but `Started` or if writing fails.
    pub(super) async fn prepare(mut self, blob_tx_hash: BlobTxHash) -> eyre::Result<Self> {
        match self.state {
            State::Started {
                sequencer_height,
                last_submission,
            } => {
                self.state = State::new_prepared(sequencer_height, last_submission, blob_tx_hash);
            }
            State::Fresh
            | State::Prepared {
                ..
...
mod tests {
    ...
    async fn should_parse_started() {
        ...
        let parsed = SubmissionState::new_from_path(file.path()).await.unwrap();
        let expected = State::new_started(
