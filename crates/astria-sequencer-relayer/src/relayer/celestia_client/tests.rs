...
fn tx_hash_from_good_response_should_succeed() {
    ...
    let extracted_tx_hash = hex_encoded_tx_hash_from_response(Ok(response)).unwrap();
...
fn tx_hash_from_bad_response_should_fail() {
    ...
    let error = hex_encoded_tx_hash_from_response(Err(Status::internal(""))).unwrap_err();
    // allow: `assert!(matches!(..))` provides poor feedback on failure.
    #[allow(clippy::manual_assert)]
    ...
    let error = hex_encoded_tx_hash_from_response(response).unwrap_err();
    // allow: `assert!(matches!(..))` provides poor feedback on failure.
    #[allow(clippy::manual_assert)]
