...
#[instrument(skip_all)]
pub(crate) fn check_chain_id_mempool<S: StateRead>(
    tx: &SignedTransaction,
    state: &S,
    immutable_data: &ImmutableData,
) -> anyhow::Result<()> {
    let chain_id = state.get_chain_id(immutable_data);
...
pub(crate) async fn check_balance_mempool<S: StateRead>(
    ...
    check_balance_for_total_fees_and_transfers(tx, state, immutable_data)
        .await
...
#[instrument(skip_all)]
pub(crate) fn get_fees_for_transaction<S: StateRead>(
    tx: &UnsignedTransaction,
    state: &S,
    immutable_data: &ImmutableData,
) -> anyhow::Result<HashMap<asset::IbcPrefixed, u128>> {
    // let transfer_fee = state
    //     .get_transfer_base_fee(immutable_data)
    //     .await
    //     .context("failed to get transfer base fee")?;
    let transfer_fee = immutable_data.fees.transfer_base_fee;
    // let ics20_withdrawal_fee = state
    //     .get_ics20_withdrawal_base_fee(immutable_data)
    //     .await
    //     .context("failed to get ics20 withdrawal base fee")?;
    let ics20_withdrawal_fee = immutable_data.fees.ics20_withdrawal_base_fee;
    // let init_bridge_account_fee = state
    //     .get_init_bridge_account_base_fee(immutable_data)
    //     .await
    //     .context("failed to get init bridge account base fee")?;
    let init_bridge_account_fee = immutable_data.fees.init_bridge_account_base_fee;
    // let bridge_lock_byte_cost_multiplier = state
    //     .get_bridge_lock_byte_cost_multiplier(immutable_data)
    //     .await
    //     .context("failed to get bridge lock byte cost multiplier")?;
    let bridge_lock_byte_cost_multiplier = immutable_data.fees.bridge_lock_byte_cost_multiplier;
    // let bridge_sudo_change_fee = state
    //     .get_bridge_sudo_change_base_fee(immutable_data)
    //     .await
    //     .context("failed to get bridge sudo change fee")?;
    let bridge_sudo_change_fee = immutable_data.fees.bridge_sudo_change_fee;
...
pub(crate) async fn check_balance_for_total_fees_and_transfers<S: StateRead>(
    ...
    let mut cost_by_asset =
        get_fees_for_transaction(tx.unsigned_transaction(), state, immutable_data)
...
fn sequence_update_fees<S: StateRead>(
    state: &S,
    fee_asset: &asset::Denom,
    fees_by_asset: &mut HashMap<asset::IbcPrefixed, u128>,
    data: &[u8],
    ...
    let fee = crate::sequence::calculate_fee_from_state(data, state, immutable_data)
...
//                 transfer_fee
//                     + crate::sequence::calculate_fee_from_state(&data, &state_tx) .await
//                       .unwrap(),
//             )
//             .await
//             .unwrap();
