...
pub(crate) async fn check_nonce_mempool<S: StateRead>(
    ...
    let signer_address = state
        .try_base_prefixed(&tx.verification_key().address_bytes(), cache)
        .await
        .context(
    ...
    let curr_nonce = state
        .get_account_nonce(signer_address, cache)
        .await
...
pub(crate) async fn check_balance_for_total_fees_and_transfers<S: StateRead>(
    ...
    let mut cost_by_asset = get_fees_for_transaction(tx.unsigned_transaction(), state, cache)
        .await
    ...
    for (asset, total_fee) in cost_by_asset {
        let balance = state
            .get_account_balance(tx, asset, cache)
            .await
