...
pub(crate) async fn check_and_execute<S: StateWrite>(
    ...
    let curr_nonce = state
        .get_account_nonce(tx.address_bytes(), cache)
        .await
    ...
    // check_balance_for_total_fees_and_transfers(tx, &state, cache)
    //     .await
    //     .context("failed to check balance for total fees and transfers")?;

    if state
        .get_bridge_account_rollup_id(tx, cache)
        .await
