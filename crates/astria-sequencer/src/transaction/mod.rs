...
pub(crate) async fn check_stateless(tx: &SignedTransaction) -> anyhow::Result<()> {
    ensure!(!tx.actions().is_empty(), "must have at least one action");
...
pub(crate) async fn check_and_execute<S: StateWrite>(
    ...
    let chain_id = state.get_chain_id(immutable_data);
        ...
        tx.chain_id() == chain_id.as_str(),
        InvalidChainId(tx.chain_id().to_string())
    );
    ...
    ensure!(curr_nonce == tx.nonce(), InvalidNonce(tx.nonce()));
    ...
    if state
        .get_bridge_account_rollup_id(tx)
        .await
        .context("failed to check account rollup id")?
        .is_some()
    {
        state.put_last_transaction_hash_for_bridge_account(tx, &tx.sha256_of_proto_encoding());
    }

    let from_nonce = state
        .get_account_nonce(tx)
        .await
    ...
    state
        .put_account_nonce(tx, next_nonce)
    ...
    for action in tx.actions() {
        match action {
            Action::Transfer(act) => act
                .check_and_execute(&mut state, immutable_data, from)
                .await
