...
async fn do_not_trigger_cleaning() {
    ...
    // (signed by sudo to create tx, but change sudo after tx is created so it fails execution)
    let tx_fail = fixture
        .checked_tx_builder()
        .with_action(FeeChange::Transfer(FeeComponents::new(10, 0)))
        .build()
...
async fn maintenance_recosting_promotes() {
    ...
    let mut fixture = Fixture::uninitialized(None).await;
    // Provide Alice with normal balance and Bob with low balance.
    fixture
        .chain_initializer()
        .with_genesis_accounts(vec![(*ALICE_ADDRESS, 10_u128.pow(19)), (*BOB_ADDRESS, 1)])
        .init()
        .await;
    ...
    bob_funds.insert(nria().into(), 1);
    let mut tx_cost = HashMap::new();
    tx_cost.insert(nria().into(), 2);
    let mempool = fixture.mempool();
    mempool
