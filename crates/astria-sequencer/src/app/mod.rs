...
impl App {
    ...
    pub(crate) async fn prepare_proposal(
        ...
        let (included_tx_bytes, signed_txs_included) = self
            .prepare_proposal_tx_execution(block_size_constraints, uses_data_item_enum)
            .await
    ...
    async fn proposal_checks_and_tx_execution(
        ...
        // NOTE: After upgrade 1 has been deployed, we can remove support for legacy form of
        //       encoding txs, i.e. we can remove `uses_data_item_enum`, and the code executed when
        //       it's false.
        let tx_bytes = tx.to_raw().encode_to_vec();
        let tx_hash_base_64 =
            telemetry::display::base64(tx_hash.unwrap_or_else(|| Sha256::digest(&tx_bytes).into()))
                .to_string();
        let data_item_bytes = if uses_data_item_enum {
            DataItem::RollupTransaction(tx_bytes.into())
                .encode()
                .wrap_err("failed encoding rollup transaction")?
        } else {
            tx_bytes.into()
        };
        let tx_len = data_item_bytes.len();
        ...
        match self.execute_transaction(tx.clone()).await {
            Ok(events) => {
                ...
                {
                    validated_txs.push(data_item_bytes);
