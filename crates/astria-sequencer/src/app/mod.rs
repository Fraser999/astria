...
impl App {
            ...
            Err(error) => {
                ...
                match proposal_info {
                    Proposal::Prepare {
                        ...
                        if matches!(error, CheckedTransactionError::InvalidNonce { .. }) {
            ...
            for tx in &user_submitted_transactions {
                match self.execute_transaction(tx.clone()).await {
                    ...
                    Err(error) => {
                        ...
                        let code = if matches!(error, CheckedTransactionError::InvalidNonce { .. })
                        {
    ...
    async fn execute_transaction(
        &mut self,
        tx: Arc<CheckedTransaction>,
    ) -> std::result::Result<Vec<Event>, CheckedTransactionError> {
        let mut state_tx = self
            .state
            .try_begin_transaction()
...
async fn construct_checked_txs<S: StateRead>(
    ...
    let txs_futures = encoded_txs.iter().map(|encoded_tx| async move {
        let tx = CheckedTransaction::new(encoded_tx.clone(), state)
            .await
            .wrap_err("failed to construct checked transactions")?;
        Ok(Arc::new(tx))
    });
