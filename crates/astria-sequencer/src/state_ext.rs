...
pub(crate) trait StateReadExt: StateRead {
    #[instrument(skip_all)]
    fn get_chain_id(&self, immutable_data: &ImmutableData) -> tendermint::chain::Id {
        immutable_data.chain_id.clone()
        // let Some(bytes) = self
        //     .get_raw("chain_id")
        //     .await
    ...
    #[instrument(skip_all)]
    fn get_revision_number(&self) -> u64 {
        0
        // let Some(bytes) = self
...
//                 .await
//                 .expect("a second storage version was written and must exist inside the
// database"),             storage_version_orig,
