...
pub(crate) trait StateReadExt: StateRead {
    ...
    async fn get_native_asset(&self) -> Result<asset::TracePrefixed> {
        ...
        let asset = std::str::from_utf8(&bytes)
            .context("bytes stored in state not utf8 encoded")?
            .parse::<asset::TracePrefixed>()
            .context("failed to parse bytes retrieved from state as trace prefixed IBC asset")?;
        Ok(asset)
    }
    ...
    #[instrument(skip_all)]
    async fn is_allowed_fee_asset<TAsset>(&self, asset: TAsset, cache: &Cache) -> Result<bool>
    where
        TAsset: Into<asset::IbcPrefixed> + std::fmt::Display + Send,
    {
        let key_str = fee_asset_key(asset);
        let key = key_str.as_bytes().to_vec();
        if let Some(Cached::IsAllowedFeeAsset(is_allowed)) = cache.get(&key) {
        ...
        let is_allowed = self
            ...
            .is_some();
        cache.put(key, Cached::IsAllowedFeeAsset(is_allowed));
        Ok(is_allowed)
    }
...
pub(crate) trait StateWriteExt: StateWrite {
        ...
        TAsset: Into<asset::IbcPrefixed> + std::fmt::Display + Send,
    {
        ...
        let block_fees_key_str = block_fees_key(asset);
        let key = block_fees_key_str.as_bytes().to_vec();

        let current_amount = {
            if let Some(Cached::BlockFee(fee)) = cache.get(&key) {
                fee
            } else {
        ...
        cache.put(key.clone(), Cached::BlockFee(new_amount));
        self.nonverifiable_put_raw(key, new_amount.to_be_bytes().to_vec());
...
//         assert_eq!(
//             state.get_native_asset().await.expect(
//                 "a native asset denomination update was written and must exist inside the
// database"             ),
//             denom_update,
...
//                 .await
//                 .unwrap()
//                 .expect("an additional ibc asset was written and must exist inside the
// database"),             denom_1.unwrap_trace_prefixed(),
