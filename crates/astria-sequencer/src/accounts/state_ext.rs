...
pub(crate) trait StateReadExt: StateRead + crate::assets::StateReadExt {
    ...
    {
        let key_str = balance_storage_key(address, asset);
        let key = key_str.as_bytes().to_vec();
        if let Some(Cached::Balance(balance)) = cache.get(&key) {
            return Ok(balance);
        }
        let Some(bytes) = self
            .get_raw(&key_str)
            ...
            // todo(Fraser): should cache this?
            return Ok(0);
        };
        let Balance(balance) = Balance::try_from_slice(&bytes).context("invalid balance bytes")?;
        cache.put(key, Cached::Balance(balance));
        Ok(balance)
    }

    #[instrument(skip_all)]
    async fn get_account_nonce<T: AddressBytes>(&self, address: T, cache: &Cache) -> Result<u32> {
        let key_str = nonce_storage_key(address);
        let key = key_str.as_bytes().to_vec();
        if let Some(Cached::Nonce(nonce)) = cache.get(&key) {
            return Ok(nonce);
        }
        let bytes = self
            .get_raw(&key_str)
        ...
        cache.put(key, Cached::Nonce(nonce));
        Ok(nonce)
    }

    #[instrument(skip_all)]
    async fn get_transfer_base_fee(&self, cache: &Cache) -> Result<u128> {
        let key = TRANSFER_BASE_FEE_STORAGE_KEY.as_bytes().to_vec();
        if let Some(Cached::TransferBaseFee(fee)) = cache.get(&key) {
            return Ok(fee);
        }
        let bytes = self
        ...
        cache.put(key, Cached::TransferBaseFee(fee));
        Ok(fee)
    }
}
...
pub(crate) trait StateWriteExt: StateWrite {
        ...
        TAsset: Into<asset::IbcPrefixed> + std::fmt::Display + Send,
    {
        ...
        let balance = self
            .get_account_balance(&address, asset, cache)
            .await
...
//                 .expect("a new nonce was written and must exist inside the database"),
//             nonce_expected,
//             "writing to a different account's nonce should not affect a different account's
// nonce"         );
//     }
//
//     #[tokio::test]
