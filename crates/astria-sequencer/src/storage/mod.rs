...
impl Storage {
    ...
    // pub(crate) fn new_delta_of_latest_snapshot(&self) -> Delta<> {
    ...
    pub(crate) fn commit_batch(&self, batch: StagedWriteBatch) -> astria_eyre::Result<RootHash> {
...
async fn get<S, K>(state: &S, key: K) -> anyhow::Result<Option<StoredValue<'static>>>
where
...
async fn nonverifiable_get<S, K>(state: &S, key: K) -> anyhow::Result<Option<StoredValue<'static>>>
where
...
// fn put<S, K>(state: &mut S, key: K, value: &StoredValue) -> anyhow::Result<()>
// where
...
// fn nonverifiable_put<S, K>(state: &mut S, key: K, value: &StoredValue) -> anyhow::Result<()>
// where
...
fn display_nonverifiable_key(key: &[u8]) -> String {
