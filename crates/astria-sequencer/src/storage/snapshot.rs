...
type VerifiableCache = Arc<QuickCache<String, CachedValue>>;
/// An in-memory cache of objects that belong in the non-verifiable store.
type NonVerifiableCache = Arc<QuickCache<Vec<u8>, CachedValue>>;
...
impl Snapshot {
    pub(super) fn new(inner: cnidarium::Snapshot) -> Self {
        Self {
            ...
            non_verifiable_cache: Arc::new(QuickCache::new(500_000)),
        }
    }
...
impl StateRead for Snapshot {
    ...
    fn prefix_raw(&self, prefix: &str) -> Self::PrefixRawStream {
        ...
        tokio::spawn(inner_snapshot.prefix_keys(prefix).try_for_each(move |key| {
            ...
            async move {
                let value = get_raw(key.clone(), inner_snapshot, cache)
                    .await?
...
fn get_raw(
    ...
    SnapshotFuture::new(async move {
        let serialized_value = cache
            .get_or_insert_async(&key, get(&inner_snapshot, &key).ok_into::<CachedValue>())
            .await?
            .into();
        Ok(serialized_value)
    })
...
fn non_verifiable_get_raw(
    ...
    SnapshotFuture::new(async move {
        let serialized_value = cache
            .get_or_insert_async(
                &key,
                non_verifiable_get(&inner_snapshot, &key).ok_into::<CachedValue>(),
            )
            .await?
            .into();
        Ok(serialized_value)
...
mod tests {
    ...
    async fn get_raw_should_succeed() {
        ...
        fn assert_in_cache(snapshot: &Snapshot, value: &[u8]) {
            let CachedValue::Stored {
                serialized_value,
            } = snapshot.verifiable_cache.get(V_KEY).unwrap()
            else {
    ...
    async fn nonverifiable_get_raw_should_succeed() {
        ...
        fn assert_in_cache(snapshot: &Snapshot, value: &[u8]) {
            let CachedValue::Stored {
                serialized_value,
            } = snapshot.non_verifiable_cache.get(NV_KEY).unwrap()
            else {
                panic!("should have value in cache");
            };
            assert_eq!(value.to_vec(), serialized_value);
