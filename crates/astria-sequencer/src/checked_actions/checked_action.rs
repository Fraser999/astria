...
/// This is an enum of all the various checked action types
#[derive(Debug)]
...
impl CheckedAction {
    ...
    pub(crate) async fn execute_and_pay_fees<S: StateWrite>(
        &self,
        mut state: S,
...
async fn fee<'a, F, S>(
    action: &'a F,
    state: &S,
) -> Result<Option<(&'a Denom, u128)>, CheckedActionError>
where
    F: FeeHandler,
    S: StateRead,
    FeeComponents<F>: TryFrom<StoredValue<'a>, Error = Report>,
{
    let fees = state
        ...
        .await
        .map_err(|source| {
            CheckedActionError::internal(&format!("error fetching {} fees", action.name()), source)
        })?
        .ok_or_else(|| CheckedActionError::ActionDisabled {
    ...
    let is_allowed_fee_asset = state
        ...
        .await
        .map_err(|source| {
            CheckedActionError::internal("failed to check allowed fee assets in state", source)
        })?;
    if !is_allowed_fee_asset {
        return Err(CheckedActionError::FeeAssetIsNotAllowed {
...
where
    ...
    FeeComponents<F>: TryFrom<StoredValue<'a>, Error = Report>,
{
    let Some((fee_asset, total_fee)) = fee(action, &state).await? else {
        // If the action has no associated fee asset, there are no fees to pay.
        return Ok(());
    };
    ...
    state
        ...
        .map_err(|source| {
            if source.to_string().contains("insufficient funds") {
                ...
                CheckedActionError::internal("failed to decrease balance for fee payment", source)
            }
        })
}
