...
impl CheckedAction {
    pub(super) fn new_rollup_data_submission(
    ...
    pub(super) async fn new_bridge_lock<S: StateRead>(
        ...
        let checked_action =
            CheckedBridgeLock::new(action, tx_signer, tx_id, position_in_tx, state).await?;
    ...
    pub(super) async fn new_bridge_transfer<S: StateRead>(
        ...
        let checked_action =
            CheckedBridgeTransfer::new(action, tx_signer, tx_id, position_in_tx, state).await?;
    ...
    pub(super) async fn new_recover_ibc_client<S: StateRead>(
        action: RecoverIbcClient,
        tx_signer: [u8; ADDRESS_LEN],
        state: S,
    ) -> Result<Self> {
        let checked_action = CheckedRecoverIbcClient::new(action, tx_signer, state).await?;
    ...
    pub(super) async fn execute<S: StateWrite>(&self, state: S) -> Result<()> {
        match self {
            ...
            Self::RecoverIbcClient(checked_action) => checked_action.execute(state).await,
        }
    }
}
