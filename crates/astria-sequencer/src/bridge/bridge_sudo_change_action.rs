...
impl ActionHandler for BridgeSudoChangeAction {
    ...
    async fn check_and_execute<S: StateWrite>(&self, mut state: S, cache: &Cache) -> Result<()> {
        ...
        if let Some(new_withdrawer_address) = &self.new_withdrawer_address {
            ...
            state
                .is_allowed_fee_asset(&self.fee_asset, cache)
                .await
        ...
        state
            .decrease_balance(self.bridge_address, &self.fee_asset, fee, cache)
            .await
