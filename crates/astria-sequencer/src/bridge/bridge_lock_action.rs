...
impl ActionHandler for BridgeLockAction {
    ...
    async fn check_and_execute<S: StateWrite>(
        ...
        state
            .ensure_base_prefix(&self.to, immutable_data)
            .await
        ...
        let transfer_fee = state.get_transfer_base_fee(immutable_data);
        ...
        let byte_cost_multiplier = state.get_bridge_lock_byte_cost_multiplier(immutable_data);
...
//             + get_deposit_byte_len(&Deposit::new( bridge_address, rollup_id, 100, asset.clone(),
