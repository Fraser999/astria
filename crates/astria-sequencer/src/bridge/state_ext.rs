...
pub(crate) trait StateReadExt: StateRead + address::StateReadExt {
    ...
    #[instrument(skip_all)]
    async fn get_last_transaction_hash_for_bridge_account<T: AddressBytes>(
        &self,
        address: &T,
    ) -> Result<Option<[u8; 32]>> {
        let Some(bytes) = self
        ...
        StoredValue::deserialize(&bytes)
            .and_then(|value| {
                storage::TransactionHash::try_from(value)
                    .map(|stored_tx_hash_bytes| Some(<[u8; 32]>::from(stored_tx_hash_bytes)))
            })
...
pub(crate) trait StateWriteExt: StateWrite {
    ...
    fn put_last_transaction_hash_for_bridge_account<T: AddressBytes>(
        ...
        let bytes = StoredValue::TransactionHash((&tx_hash).into())
            .serialize()
...
mod test {
    ...
    #[tokio::test]
    async fn last_transaction_hash_for_bridge_account_round_trip() {
        let storage = cnidarium::TempStorage::new().await.unwrap();
        let snapshot = storage.latest_snapshot();
        let mut state = StateDelta::new(snapshot);
        ...
        let tx_hash = [2; 32];
        state
            .put_last_transaction_hash_for_bridge_account(&bridge_address, tx_hash)
            .unwrap();
        let retrieved_tx_hash = state
            .get_last_transaction_hash_for_bridge_account(&bridge_address)
            .await
            .unwrap();
