...
pub(crate) trait StateReadExt: StateRead + address::StateReadExt {
    ...
    async fn get_bridge_account_rollup_id<T: AddressBytes>(
        ...
        let key_str = rollup_id_storage_key(&address);
        let key = key_str.as_bytes().to_vec();
        if let Some(Cached::RollupId(rollup_id)) = cache.get(&key) {
            return Ok(Some(rollup_id));
        }
        ...
        cache.put(key, Cached::RollupId(rollup_id));
        Ok(Some(rollup_id))
    }
