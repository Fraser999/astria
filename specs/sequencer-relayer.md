...
attempt. `sequencer_height` indicates the highest sequencer block included in
the attempt, while `last_submission` records the last successful submission. The
details of the last submission include the Celestia block height at which the
submission was stored, and the highest sequencer block included in the
submission.
...
This state is written by the relayer each submission has been prepared and is
...
the maximum of one minute after the `at` timestamp, and 15 seconds.
