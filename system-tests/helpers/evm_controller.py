import requests
import time
from .utils import run_subprocess, wait_for_statefulset_rollout
...
class EvmController:
    ...
    def wait_until_chain_at_height(self, height, timeout_secs):
        ...
        while True:
            ...
            print(
                ...
                f"{height}, {timeout_instant - now:.3f} seconds remaining",
            )
            time.sleep(1)
        print(
    ...
    def _wait_for_deploy(self):
        wait_for_statefulset_rollout("rollup", "astria-geth", "astria-dev-cluster", 600)
        # Wait for the ingress to be ready.
        args = [
            "kubectl", "wait", "-n=astria-dev-cluster", "ingress/astria-rpc-ingress",
            "--for=jsonpath={.status.loadBalancer.ingress}", f"--timeout=600s"
        ]
        run_subprocess(args, "waiting for ingress on rollup")
    ...
    def _try_send_json_rpc_request(self, method, *params):
        ...
        try:
            ...
            print(f"rollup: failed request: {payload}")
            raise
