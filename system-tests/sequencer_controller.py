...
class SequencerController:
    ...
    def deploy_sequencer(
        ...
        If `upgrade_name` is set, then the chart value `genesis.postLatestUpgrade` will be set to
        false to ensure genesis.json and upgrades.json have appropriate values set for pre-upgrade.
        ...
        if enable_persistent_storage:
            args.append(f"--set=storage.enabled=true")
        if upgrade_name:
            args.append("--set=genesis.postLatestUpgrade=false")
        ...
        self._try_subprocess(args, err_msg="failed to deploy")
        self._wait_for_deploy(timeout_duration=600)
    ...
    def stage_upgrade(self, image_tag, upgrade_name, activation_height):
        ...
        """
        try:
            self.last_block_height_before_restart = self._try_get_last_block_height()
        except:
            self.last_block_height_before_restart = None

        # Update the upgrades.json file with the specified activation height.
        helm_args = [
            ...
            f"--set=sequencer.upgrades.{upgrade_name}.activationHeight={activation_height}"
        ]
        print(f"Running `{' '.join(map(str, helm_args))}`")
        helm_result = subprocess.run(helm_args, capture_output=True)
        if helm_result.returncode != 0:
            raise SystemExit(
                f"{self.name} failed to prepare the upgrade configmap:"
                f"\n{helm_result.stdout}\n{helm_result.stderr}")

        configmap = helm_result.stdout
        kubectl_args = ["kubectl", "apply", f"-n={self.namespace}", "--filename=-"]
        print(f"Running `{' '.join(map(str, kubectl_args))}`")
        kubectl_result = subprocess.run(kubectl_args, input=configmap, capture_output=False)
        if kubectl_result.returncode != 0:
            raise SystemExit(f"{self.name} failed to apply the upgrade configmap")

        # Upgrade the image to use the new binary.
        self._try_subprocess(
            [
                "kubectl", "set", "image", f"-n={self.namespace}",
                "statefulset", "sequencer", f"sequencer=ghcr.io/astriaorg/sequencer:{image_tag}"
            ],
            err_msg="failed to deploy upgrade image"
        )
    ...
    def wait_for_upgrade(self, upgrade_activation_height):
        ...
        self._wait_for_deploy(timeout_duration=40)
        ...
        # the upgrade.
        latest_block_height = self.get_last_block_height()
        timeout_duration = max(upgrade_activation_height - latest_block_height, 1) * 5
        self.wait_until_chain_at_height(upgrade_activation_height, timeout_duration)
    ...
    def get_vote_extensions_enable_height(self):
        ...
        Exits the process on error.
        """
        response = self._try_send_json_rpc_request_with_retry("consensus_params")
    ...
    def wait_until_chain_at_height(self, height, timeout_duration):
        ...
        while True:
            ...
            if time.monotonic() >= timeout:
                raise SystemExit(
                    f"{self.name} failed to reach block {height} within {timeout_duration} "
    ...
    def _try_subprocess(self, args, err_msg):
        print(f"Running `{' '.join(map(str, args))}`")
        try:
            subprocess.check_call(args)
        except subprocess.CalledProcessError:
            raise SystemExit(f"{self.name} {err_msg}")

    def _wait_for_deploy(self, timeout_duration):
        print(f"Waiting for {self.name} to deploy")
        args = [
            "kubectl", "rollout", "status", "statefulset/sequencer",
            f"-n={self.namespace}", f"--timeout={timeout_duration}s"
        ]
        print(f"Running `{' '.join(map(str, args))}`")
        status_check = subprocess.run(args, capture_output=False)
        if status_check.returncode == 0:
            return

        print(
            f"{self.name} failed to deploy the sequencer within {timeout_duration} seconds "
        ...
        subprocess.run(
            ["kubectl", "get", "pods", f"-n={self.namespace}"],
            capture_output=False
        )
        print()
        subprocess.run(
            [
                "kubectl", "events", f"-n={self.namespace}", "--for=Pod/sequencer-0",
                "--types=Warning"
            ],
            capture_output=False
        )
    ...
    def _try_send_json_rpc_request(self, method, *params, verbose=True):
        ...
        try:
            response = requests.post(
                f"http://localhost:{self.rpc_local_port}", json=payload
            ).json()
        except Exception as error:
            if verbose:
                print(f"{self.name}: request: {payload}")
            self._terminate_port_forwarding()
            raise error
